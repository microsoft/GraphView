...
namespace GraphView
{
    public abstract class GraphOptimizer
    {
        ...
        private const int MaxStates = 1000;
        ...
        public virtual List<Tuple<MatchNode, MatchEdge>> GetOptimizedTraversalOrder(ConnectedComponent subGraph)
        {
            ...
            // If exist, pick a node without incoming edges as the start point
            ...
            while (componentStates.Any())
            {
                ...
                foreach (var curComponent in componentStates)
                {
                    ...
                    if (nextCompnentStates.Count >= MaxStates)
                        throw new GraphViewException("This graph patter is not supported yet.");
