...
namespace GraphView
{
    public class DirectedSparseGraph<T> : IGraph<T> where T : IComparable<T>
    {
        ...
        HashSet<String> vertexMarkSet = new HashSet<String>();

        public void BFSPartitionGraph(Dictionary<String, String> _vertexPartition, int partitionNum)
        {
            foreach(var v in _adjacencyList)
            {
                var vID = v.Key;
                if(!vertexMarkSet.Contains(vID.ToString()))
                {
                    BreadthFirstWalkAssignPartition(vID, _vertexPartition, partitionNum);
                }
            }
        }

        public int getMinLoadPartitionIndex(int[] partitionLoad)
        {
        ...
        public virtual IEnumerable<T> BreadthFirstWalkAssignPartition(T source, Dictionary<String, String> vertexPartition, int partitionNum)
        {
            //partitionLoad[partition]++;
            int[] partitionLoad = new int[partitionNum];
            ...
            var visited = new HashSet<T>();
            var queue = new GraphViewExecutionRuntime.Graph.Queue<T>();
            var listOfNodes = new ArrayList<T>(VerticesCount);
            ...
            queue.Enqueue(source);
            int partitionIndex = getMinLoadPartitionIndex(partitionLoad);
            vertexPartition.Add(source.ToString(), partitionIndex.ToString());
            vertexMarkSet.Add(source.ToString());
            partitionLoad[partitionIndex]++;
            while (!queue.IsEmpty)
            {
                var current = queue.Dequeue();
                var neighbors = Neighbours(current);
                partitionIndex = getMinLoadPartitionIndex(partitionLoad);
                foreach (var adjacent in neighbors)
                {
                    if (!visited.Contains(adjacent))
                    {
                        //listOfNodes.Add(adjacent); // Avoid cost, not store the traversal path.
                        vertexPartition.Add(adjacent.ToString(), partitionIndex.ToString());
                        vertexMarkSet.Add(adjacent.ToString());
                        partitionLoad[partitionIndex]++;
                        visited.Add(adjacent);
                        queue.Enqueue(adjacent);
                    }
